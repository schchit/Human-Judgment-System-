# 文档索引 / Documentation Index

欢迎来到 Human Judgment System (HJS) 文档中心！

Welcome to the Human Judgment System (HJS) documentation center!

## 📚 文档导航 / Documentation Navigation

### 📄 白皮书 / Whitepapers

- **[English Whitepaper (PDF)](whitepaper/en/White%20Paper%20on%20Human%20Judgment%20System%20v1.0%20(English%20Version).pdf)**
  - Complete conceptual framework and technical specifications
  - 完整的概念框架和技术规范

- **[中文白皮书 (PDF)](whitepaper/zh-CN/人类判断系统白皮书%20v1.0（中文版）.pdf)**
  - 完整的概念框架和技术规范
  - Complete conceptual framework and technical specifications

## 🗂️ 文档结构 / Document Structure

```
docs/
├── README.md              # 本文档（文档索引）
├── whitepaper/           # 白皮书目录
│   ├── en/               # 英文版白皮书
│   └── zh-CN/            # 中文版白皮书
└── [未来更多文档]        # Future documentation
```

## 📖 阅读建议 / Reading Recommendations

### 对于初学者 / For Beginners

1. 从 [项目主 README](../README.md) 开始，了解项目概述
2. 阅读白皮书中的"核心概念"部分
3. 查看"应用场景"了解实际用例

### 对于开发者 / For Developers

1. 仔细阅读完整白皮书
2. 理解判断层的技术架构
3. 查看实现示例（如有）

### 对于研究者 / For Researchers

1. 深入阅读白皮书的理论部分
2. 关注责任蒸发问题的学术讨论
3. 思考如何扩展和改进协议

## 🔄 文档更新 / Document Updates

文档会随着项目的发展持续更新。最新版本请查看：

Documentation will be continuously updated as the project evolves. For the latest version, please check:

- [GitHub Releases](https://github.com/schchit/Human-Judgment-System/releases)
- [GitHub Discussions](https://github.com/schchit/Human-Judgment-System/discussions)

## 💡 反馈与建议 / Feedback & Suggestions

如果您对文档有任何建议或发现问题，请：

If you have any suggestions or find issues with the documentation, please:

- 在 [GitHub Issues](https://github.com/schchit/Human-Judgment-System/issues) 中报告
- 在 [GitHub Discussions](https://github.com/schchit/Human-Judgment-System/discussions) 中讨论
- 直接提交 Pull Request 改进文档

---

[← 返回主页](../README.md) | [贡献指南](../CONTRIBUTING.md)
